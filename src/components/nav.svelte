...
<header bind:this="{container}">
	<div class="header__inner">
		<div class="header__logo">
			{#if pathName == '/'}
				<a href="/" title="Maison Matador home" rel="internal" data-sveltekit-noscroll>
					<h1 class="header__logo__inner">
						<span class="visually-hidden"> Maison Matador </span>
						<svg role="presentation" aria-hidden="true"><use xlink:href="#maison-logo"> </use></svg>
					</h1>
				</a>
			{:else}
				<a
		...
		<div class="header__menu-drawer">
			...
			>
				...
				>
					<span class="header__menu-icon-top-bar"> </span>
					<span class="header__menu-icon-middle-bar"> </span>
					<span class="header__menu-icon-bottom-bar"> </span>
				...
				>
					<nav class="menu-drawer">
						<div class="menu-drawer__inner">
							<ul class="menu-drawer__items" class:show-nav-items="{showNavItems}">
								{#each navLinks as navItem, index}
									<li class="menu-drawer__item" style="transition-delay: {0.1 * index + 's'}">
										{#if navItem.navigationItemUrl.linkType == 'external'}
											<a
												...
												rel="external"
												title="Go to external link at {navItem.navigationItemUrl.href}"
											>
												{navItem.text}
											</a>
										{/if}
										...
										{#if navItem.navigationItemUrl.linkType == 'internal'}
											<a
												...
												on:click="{closeMenuDrawer}"
												class="menu-drawer__link"
												href="{navItem.navigationItemUrl.internalLink.handle.current}"
												rel="internal"
			...
			<nav class="menu-list">
				<div class="menu-list__inner">
					<ul class="menu-list__items" class:show-nav-items="{showNavItems}">
						{#each navLinks as navItem, index}
							<li class="menu-list__item" style="transition-delay: {0.1 * index + 's'}">
								{#if navItem.navigationItemUrl.linkType == 'external'}
									<a
										on:click="{closeMenuDrawer}"
...
<style>
	...
	.header-menu-drawer.menu-closing {
		animation: headerDrawerClose var(--duration-default) cubic-bezier(0.11, 0, 0.5, 0) forwards;
		pointer-events: none;
	}
